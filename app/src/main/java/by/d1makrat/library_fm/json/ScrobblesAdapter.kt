    ...
    override fun read(jsonReader: JsonReader): ScrobblesJsonModel {
        ...
        for (i in 0 until scrobblesJsonArray.size()) {
            ...
            if (!scrobbleJsonObject.has(ATTRIBUTE_KEY)) {//TODO parse and show scrobble that "now playing"
                ...
                val imageUrl = scrobbleJsonObject.get(IMAGE_KEY).asJsonArray.get(MAX_IMAGE_RESOLUTION_INDEX).asJsonObject.get(TEXT_KEY).asString

                scrobbles.add(Scrobble(trackTitle, artist, album, unixDate, imageUrl))
            }
        }
